...
export default class CustomCrosstoolDrawer {
  ...
  drawXLabel(ctx, hover) {
    ...
    if (this.config.components.xAxis.visible) {
      ctx.font =
        this.config.components.xAxis.fontSize +
        "px " +
        this.config.components.xAxis.fontFamily;
      // X hover
      const x = hover.x;
      ...
      const yTextPos =
        xAxis.y + xLabelMarginTop + fontHeight + xLabelPaddingTop - 1; // -1 for vertical adjustment
  ...
  drawMagnetPrice(ctx, hover) {
    ...
    if (this.config.components.xAxis.visible) {
      ...
      const labelText = this.chartModel.pane.regularFormatter(
        this.chartModel.priceFromY(y)
      );
      const width = ctx.measureText(labelText).width;
      const fontHeight = this.config.components.xAxis.fontSize;
      // box
      ...
      const yTextPos =
        hover.y + xLabelMarginTop + fontHeight + xLabelPaddingTop - 1; // -1 for vertical adjustment
      ctx.fillText(labelText, xTextPos, yTextPos);
      ctx.restore();
    }
  }
